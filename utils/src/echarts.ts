...
const fetchVersionsData = async () => {
  const url = "https://registry.npmmirror.com/-/v1/search?text=echarts&size=1";
...
const init = async () => {
  ...
  (async () => {
    for (const version of versions) {
      ...
      const outputPath = path.join(__dirname, "..", "lib/echarts", version);

      const filePath = path.join(outputPath, "echarts.min.js");
      try {
