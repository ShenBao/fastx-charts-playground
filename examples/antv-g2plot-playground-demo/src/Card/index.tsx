...
const Card = ({ index }: { index: number }) => {
  ...
  const init = async () => {
    ...
    const list = [
      `https://registry.npmmirror.com/@antv/g2plot/${item.version}/files/dist/g2plot.min.js`,
      ...(item?.scripts || []),
    ];
    ...
    // 将 G2Plot 转换为 为 g2plot，供 编译后的代码使用
    // 增加自己的全局变量，用于 DEMO 中的依赖，以 G2Plot 为例
    sandbox.execScriptInSandbox(`
      window.g2plot = window.G2Plot;
    `);
